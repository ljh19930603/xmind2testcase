...
def xmind_to_zentao_csv_file(xmind_file):
    ...
    fileheader = ["用例编号","所属模块", "用例标题", "前置条件", "步骤", "预期", "关键词", "优先级", "用例类型", "适用阶段"]
...
def gen_a_testcase_row(testcase_dict):
    ...
    case_title = gen_case_title(testcase_dict['name'], testcase_dict['sheet_name'])
    case_id = gen_case_id(testcase_dict['name'])
    ...
    row = [case_id, case_module, case_title, case_precontion, case_step, case_expected_result, case_keyword, case_priority, case_type, case_apply_phase]
    return row

def gen_case_title(case_name, sheet_name=""):
    result = search(r"(.*)\(#", case_name)
    if result:
        return f"【{sheet_name}】{result.group(1)}"
    else:
        return f"【{sheet_name}】{case_name}"

def gen_case_id(case_name):
    result = search(r"#(\d+)", case_name)
    if result:
